...
test_input = prepare_sentence_tokens(
    hf_minilm_model, "run pytorch gpt2 model on huggingface with iree in five steps;first step, load pretrained model with transformers;second step,wrap pretrained model with shark_turbine.aot.compiledmodule,third step,export mlir using get_mlir_module;and fourth step,compile mlir into iree using iree-compiler.compiler_str;last step,save iree model in vmfb format"
)
