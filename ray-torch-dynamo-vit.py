...
class Unit:
    """
    Represents a unit in system.

    Attributes:
        output (str): The output of the unit.
        input_names (list): A list of input names for the unit.
    """
    ...
    def __init__(self, output: str, input_names: list) -> None:
            ...
            Args:
                output (str): The output value.
                input_names (list): The list of input names.
            """
            self.output = output
...
class ExecutableUnit(Unit):
    ...
    def print_debug(self):
        '''
        Prints the debug information of the ExecutableUnit.
        '''
        print('===========================graph', ExecutableUnit.exec_unit_count,
              ':', self.output, '================================')
    ...
    def lower_to_iree(self):
        '''
        Lowers the ExecutableUnit to IREE (Internet Real-Time Execution Environment).
        '''
    ...
    def upload_graph_to_ray(self):
        '''
        Uploads the graph of the ExecutableUnit to Ray.
        '''
        self.graph_ref = ray.put(self.graph)

    def run_on_ray(self, input):
        '''
        Runs the ExecutableUnit on Ray with the given input.

        Args:
            input: The input for the ExecutableUnit.

        Returns:
            The result of running the ExecutableUnit on Ray.
        '''
        debug = False
        if debug:
    ...
    def run_only_sharding(self, input):
        '''
        Runs the ExecutableUnit with only sharding on the given input.

        Args:
            input: The input for the ExecutableUnit.

        Returns:
            The result of running the ExecutableUnit with only sharding.
        '''
        start = time.time()
...
class PipeStageWithLinear(Unit):
    ...
    def __init__(self, output: str, input_names: list, stages: list = None, env: dict = None) -> None:
        """
        Initialize the PipeStageWithLinear class.

        Args:
            output (str): The output name.
            input_names (list): The list of input names.
            stages (list): The list of stages. Defaults to None. 由pipestages and LinearStage组成
            env (dict): The environment dictionary. Defaults to None. 每个带residue的stage都需要env
        """
        super().__init__(output, input_names)
        self.stages = stages  
        self.env = env  
    ...
    def test_trace_run(self, input):
        ...
        Args:
            input: The input data for the method.
            ...
            if isinstance(self.stages[i], PipeStage):
                if len(self.stages[i].input_names) > 1:
                    if i == 0:
                        ...
                        next_input = [self.env[input_name]
                                      for input_name in self.stages[i].input_names]
...
class LinearStage(Unit):
    ...
    def run_only_sharding(self, x):
        ...
        if self.input_sharding:
            input_data_sections = input_sharding_3d(x, self.group, 2)
            if False:
                result = [self.shards[i].run_only_sharding(
                    input_data_sections[i]).detach().numpy() for i in range(len(self.shards))]
                result = torch.sum(torch.tensor(result), 0)
            else:
                for i in range(len(self.shards)):
                    result += self.shards[i].run_only_sharding(
                        input_data_sections[i])
...
# torch.save(model.state_dict(), 'vit-base-patch16-224-in21k.pth')
example_input = torch.randn(1, 3, 224, 224)


def group_reverse(group):
    group_in_single = torch.cat([torch.tensor(group[i])
...
if True:
    sp_index = 2
    split_points.append('embedding_output')
    for i in range(11):
        split_points.append('hidden_states_'+str(sp_index))
        sp_index += 6
        split_points.append('hidden_states_'+str(sp_index))
        sp_index += 3

    split_points.append('hidden_states_101')
    split_points.append('sequence_output')
else:
    sp_index = 13
    split_points.append('x_7')
    for i in range(12):
        split_points.append('x_'+str(sp_index))
        sp_index += 7
        split_points.append('x_'+str(sp_index))
        sp_index += 5
...
def stage_constructor(gm: torch.fx.GraphModule, example_inputs: List[torch.Tensor]):
    ...
    for node in gm.graph.nodes:
        ...
        if node.name in split_points:
            ...
            sub_graph.recompile()
            stage = PipeStage(node.name, input_names, 1,
                              sub_graph, sharding=sharding)  # 创建stage
    ...
    input_names, sub_graph = fix_graph_input(sub_graph, gm_env)
    sub_graph.recompile()
    stage = PipeStage(node.name, input_names, 1, sub_graph,
                      sharding=sharding)  # 创建stage
    # print(stage.__class__.__name__)
    stages.append(stage)  # 添加到中间表示图中
    # stage.graph.graph.print_tabular()
...
for i in range(12):
    ...
    att_group = read_group(
        '/home/jq/data/vit/encoder_attention_'+str(i)+'_group.pkl')
    ffn_group = read_group(
        '/home/jq/data/vit/encoder_ffn_'+str(i)+'_group.pkl')
    q_name = 'mixed_query_layer' if i==0 else 'mixed_query_layer_'+str(i)
    ...
    sub_model_env = dict()
    pipe_stage = PipeStage('', list(), 1, torch.fx.GraphModule(
        att_stage.graph, torch.fx.Graph()), dict())
    print(att_stage.output)
    pipe_linear_stage = PipeStageWithLinear(
        att_stage.output, att_stage.input_names, list(), dict())
    j = 0
    first_node = {}
    ...
    for node in att_stage.graph.graph.nodes:
        ...
        if node.name == q_name:
            ...
            sub_graph.recompile()
            sub_graph.graph.print_tabular()
            # TODO,more init param
            pipe_stage = PipeStage(last_node.name, input_names, 1, sub_graph)
            ...
            if debugging:
                ...
                linear_stage = LinearStage(node.name,
                                           [arg.name if isinstance(
                                               arg, torch.fx.Node) else arg for arg in node.args],
                                           False,
                ...
                for k in range(len(att_group)):
                    ...
                    sub_graph = torch.fx.GraphModule(
                        att_stage.graph, torch.fx.Graph())
                    # sub_graph.graph.placeholder(last_node.name)
                    sub_graph.graph.node_copy(node)
                    sub_graph.graph.output(node)
                    sub_graph.add_submodule(node.target, linear_shards[k])
                    input_names, sub_graph = fix_graph_input(
                        sub_graph, sub_model_env)
                    sub_graph.recompile()
                    linear_stage_shard = LinearStageShard(
                        node.name+'_shard_'+str(k), input_names, sub_graph)  # TODO,more init param
                    linear_stage.shards.append(linear_stage_shard)

                pipe_linear_stage.stages.append(
                    linear_stage)  # LinearStage 添加到pipeline
                # 4/4 预备好新的graph
                sub_graph = torch.fx.GraphModule(
                    att_stage.graph, torch.fx.Graph())
                ...
                sub_graph.recompile()
                # TODO,more init param
                pipe_stage = PipeStage(node.name, input_names, 1, sub_graph)
                pipe_linear_stage.stages.append(pipe_stage)  # 2/4,中间表示处理
                # 预备好新的graph
                sub_graph = torch.fx.GraphModule(
                    att_stage.graph, torch.fx.Graph())
            ...
            if last_node.op == 'call_module' and isinstance(att_stage.graph.get_submodule(node.target), torch.nn.Linear):
                ...
                if debugging:
                    ...
                    linear_stage = LinearStage(node.name,
                                               ...
                                               group=att_group)
                    # sharding here
                    linear_shards = linear_sharding(
                        linear_module, att_group, True)
                    for k in range(len(att_group)):
                    ...
                    pipe_stage = PipeStage(
                        node.name, input_names, 1, sub_graph)
                    pipe_linear_stage.stages.append(pipe_stage)  # 2/4,中间表示处理
                    # 预备好新的graph
                    sub_graph = torch.fx.GraphModule(
                        att_stage.graph, torch.fx.Graph())
            ...
            if debugging:
                ...
                # sharding here
                linear_shards = linear_sharding(
                    linear_module, att_group, False)  # HERE,split input_dim
                for k in range(len(att_group)):
                    # 3.1/4 为LinearStageShard创建新的子图
                    sub_graph = torch.fx.GraphModule(
                        att_stage.graph, torch.fx.Graph())
            ...
            if node.op == 'call_module':
                ...
                sub_graph.register_parameter(
                    node.target, att_stage.graph.get_parameter(node.target))
        last_node = node

    # process last pipestage in PipeStageWithLinear
    input_names, sub_graph = fix_graph_input(sub_graph, sub_model_env)
    sub_graph.recompile()
    pipe_stage = PipeStage(att_stage.output+'_'+last_node.name,
                           input_names, 2, sub_graph)  # TODO,more init param
    pipe_linear_stage.stages.append(pipe_stage)  # 2/4,中间表示处理
    ...
    pipe_stage = PipeStage('', list(), 1, torch.fx.GraphModule(
        ffn_stage.graph, torch.fx.Graph()), dict())
    pipe_linear_stage = PipeStageWithLinear(
        ffn_stage.output, ffn_stage.input_names, list(), dict())
    j = 0
    sub_model_env = dict()
    for node in ffn_stage.graph.graph.nodes:
        ...
        if node.name == ffn_fc1_name:
            ...
            for k in range(len(ffn_group)):
                # 3.1/4 为LinearStageShard创建新的子图
                sub_graph = torch.fx.GraphModule(
                    ffn_stage.graph, torch.fx.Graph())
                # sub_graph.graph.placeholder(last_node.name)
                sub_graph.graph.node_copy(node)
                sub_graph.graph.output(node)
                sub_graph.add_submodule(node.target, linear_shards[k])
            ...
            # sharding here
            linear_shards = linear_sharding(
                linear_module, ffn_group, False)  # HERE,split input_dim
            for k in range(len(ffn_group)):
                # 3.1/4 为LinearStageShard创建新的子图
                sub_graph = torch.fx.GraphModule(
                    ffn_stage.graph, torch.fx.Graph())
            ...
            if node.op == 'call_module':
                ...
                sub_graph.register_parameter(
                    node.target, ffn_stage.graph.get_parameter(node.target))
        last_node = node
    input_names, sub_graph = fix_graph_input(sub_graph, sub_model_env)
    sub_graph.recompile()
    # 注意:pipestagewithlinear的最后一个sub-stage,last_node.name 都为output，
    # 不同pipestagewithlinear要避免重复
    pipe_stage = PipeStage(ffn_stage.output+'_'+last_node.name,
                           input_names, 2, sub_graph)  # TODO,more init param
    pipe_linear_stage.stages.append(pipe_stage)  # 2/4,中间表示处理
...
class Worker:
    ...
    def compute(self, model_id, input, multi_input: bool = False):
        ...
        if model_id == 'output':
            ...
            output         output                        output                        ((g__model___layernorm, g__model___pooler_activation),)  {}
            '''
            results = self.models[model_id].forward(
                input.detach().numpy())[1].to_host()
        else:
            if multi_input:
                input_ = (input[i].detach().numpy() for i in range(len(input)))
...
if using_iree:
    for i in range(num_workers):
        ...
        for j in range(len(stages)):
            if isinstance(stages[j], PipeStage):
                ...
                init_rets.append(w.add_model.remote(
                    stages[j].ref, stages[j].output))
    ...
    for i in range(num_workers):
        ...
        for j in range(len(stages)):
            if isinstance(stages[j], PipeStage):
                # print(f'adding stage {stages[j].output} ')
                init_rets.append(w.add_graph.remote(
                    stages[j].graph_ref, stages[j].output))
            elif isinstance(stages[j], PipeStageWithLinear):
                for sub_stage in stages[j].stages:
                    if isinstance(sub_stage, PipeStage):
                        # print(f'adding stage {sub_stage.output} ')
                        init_rets.append(w.add_graph.remote(
                            sub_stage.graph_ref, sub_stage.output))
                    elif isinstance(sub_stage, LinearStage):
                        for shard in sub_stage.shards:
                            # print(f'adding stage {shard.output} ')
                            init_rets.append(w.add_graph.remote(
                                shard.graph_ref, shard.output))
        workers.append(w)
...
def profile_torch_ray():
    ...
    for j in range(len(stages)):
        ...
        print(
            f'Run stage: {stages[j].output} input_num:{len(stages[j].input_names)}')
        if len(stages[j].input_names) > 1:
            if j == 0:
                next_input = example_input
            else:
                next_input = [main_env[input_name]
                              for input_name in stages[j].input_names]
                # print('input len:',len(next_input))
        else:
            if j == 0:
                next_input = example_input
